...
@Component({
  selector: "app-payment-location-form-modal",
  templateUrl: "./payment-location-form-modal.component.html",
  styleUrls: ["./payment-location-form-modal.component.scss"],
  providers: [PaymentLocationService],
})
export class PaymentLocationFormModalComponent
  ...
  implements OnInit
{
  ...
  private _getInitialData() {
    if (this.isEditMode) {
      this.isLoading = true;
      this._paymentLocationService.getById(this.entryId).subscribe((res) => {
        if (res.isOk) {
